-- 12-month MoM growth heatmap for TVL by chain.
...
, monthly_avg AS (
	SELECT chain, DATE_TRUNC('month', date) AS month_start
		, AVG(tvl) AS avg_val
	FROM dune.ghrjeondata.result_defi_overview_chain_daily_metrics
	WHERE tvl IS NOT NULL AND date >= DATE_ADD('month', -13, (SELECT max_date FROM latest_day))
	GROUP BY 1, 2
)
, with_mom AS (
