...
, mom_raw AS (
	SELECT chain, month_start
		, CASE WHEN prev_val IS NULL OR prev_val < 1e3 THEN NULL ELSE (avg_val - prev_val) / prev_val END AS mom_growth
	FROM with_mom
)
, overall AS (
	SELECT chain
