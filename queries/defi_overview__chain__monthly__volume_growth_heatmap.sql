...
, volume_ytd AS (
	SELECT chain, SUM(dex_volume) AS volume_ytd
	FROM dune.ghrjeondata.result_defi_overview_chain_daily_metrics
	WHERE dex_volume IS NOT NULL
		AND date >= DATE_TRUNC('year', (SELECT max_date FROM latest_day))
	GROUP BY 1
)
, monthly_avg AS (
	SELECT chain, DATE_TRUNC('month', date) AS month_start
		, AVG(dex_volume) AS avg_val
	FROM dune.ghrjeondata.result_defi_overview_chain_daily_metrics
	WHERE dex_volume IS NOT NULL AND date >= DATE_ADD('month', -13, (SELECT max_date FROM latest_day))
	GROUP BY 1, 2
)
, with_mom AS (
...
, mom_raw AS (
	SELECT chain, month_start
		, CASE WHEN prev_val IS NULL OR prev_val < 1e6 THEN NULL ELSE (avg_val - prev_val) / prev_val END AS mom_growth
	FROM with_mom
)
, overall AS (
	SELECT chain
...
SELECT ct.chain, vy.volume_ytd, o.median_12m_growth, o.num_positive
...
FROM current_tvl ct
INNER JOIN volume_ytd vy ON ct.chain = vy.chain
