...
-- Pivoted: m-01 = latest month, m-12 = 12 months ago.
...
, latest_month AS (
	SELECT DATE_TRUNC('month', max_date) AS max_month
	FROM latest_day
)
...
, mom_raw AS (
	SELECT
		chain, month_start
		, CASE WHEN prev_val IS NULL OR prev_val < 1e-8 THEN NULL
			ELSE (avg_val - prev_val) / prev_val
		END AS mom_growth
	FROM with_mom
)
...
SELECT
	ct.chain, ct.current_tvl, o.median_12m_growth, o.num_positive
