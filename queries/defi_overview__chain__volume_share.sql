...
, monthly_totals AS (
	SELECT
		month_start
		, SUM(avg_volume) AS total_volume
	FROM monthly_chain
	GROUP BY 1
)
...
SELECT
	...
	, mc.chain
	, CASE
		WHEN mt.total_volume IS NULL OR mt.total_volume < 1e6 THEN NULL
		ELSE mc.avg_volume / mt.total_volume
