...
## REST API Fallback — Query Update

Prefer the CLI (`dune query update`, `dune query run`). If the CLI is unavailable, use the REST API directly:

```
PATCH https://api.dune.com/api/v1/query/{query_id}
Headers: X-Dune-API-Key: <key>, Content-Type: application/json
Body: {"query_sql": "<sql>", "name": "<title>", "description": "<desc>"}
```
...
req = urllib.request.Request(
    ...
    data=json.dumps({'query_sql': sql}).encode(),
...
Read `DUNE_API_KEY` from the dashboard's local `.env` file (see [upload-workflow.md](../../data-plumber/references/upload-workflow.md) for the pattern).

**Always execute after updating.** An updated query still shows stale results until re-run:

```python
req = urllib.request.Request(
    f'https://api.dune.com/api/v1/query/{query_id}/execute',
    data=json.dumps({'performance': 'medium'}).encode(),
    headers={'X-Dune-API-Key': api_key, 'Content-Type': 'application/json'},
    method='POST'
)
resp = urllib.request.urlopen(req, timeout=30)
# Returns: {"execution_id": "...", "state": "QUERY_STATE_PENDING"}
```
