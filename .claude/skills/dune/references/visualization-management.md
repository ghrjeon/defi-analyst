...
These are stable — do not probe with exploratory Python. Go directly to the data you need.

**`dune viz list --query-id <id> -o json`**:
```json
{"results": [{"id": 123, "name": "Chart Name", "type": "chart", "created_at": "...", "updated_at": "..."}], "total_count": 9}
```

**`dune viz get <id> -o json`**: full viz object with `id`, `name`, `type`, `options` (containing `seriesOptions`, `columnMapping`, `columns`, etc.).

**`dune dashboard get <id> -o json`**:
```json
{
  "dashboard_id": 212511,
  "visualization_widgets": [{"widget_id": 123, "visualization_id": 456, "position": {"row": 0, "col": 0, "size_x": 3, "size_y": 7}}],
  "text_widgets": [...]
}
```
- Widgets only contain `visualization_id` — no query_id or viz name. Cross-reference with `viz list` or local `queries.yml` to identify which query a widget belongs to.

## Efficient Update Workflow

For color or option changes:
1. `dune viz get <id> -o json` → extract `options`
2. Modify in Python (write to temp file for complex options)
3. `dune viz update <id> --options "$(cat /tmp/viz_opts.json)"` — replaces entire options object
4. One `dune viz get` to verify — don't re-fetch all vizzes

For renaming: `dune viz update <id> --name "New Name"` — no need to touch options.

## Timeseries Color Convention

Use 3 intensities per color scheme: light (raw data area/line) → medium (30d MA) → dark (7d MA).

```json
{"raw_metric": {"color": "#LIGHT", "zIndex": 0}, "metric_30d_ma": {"color": "#MEDIUM", "zIndex": 1}, "metric_7d_ma": {"color": "#DARK", "zIndex": 2}}
```

Match derived metrics to their parent's color family (e.g. volume-derived → orange, fee-derived → green). Cross-metric ratios get a distinct family. Document all schemes in a project's `styles.yml`.
